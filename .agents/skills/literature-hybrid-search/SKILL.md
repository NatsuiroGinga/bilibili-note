...
1. 在仓库根运行 `python3 -m scripts.literature_search status --json`。
2. 索引存在且未陈旧时，运行 `python3 -m scripts.literature_search query "查询" --scope local --mode hybrid --json`。
3. 向量依赖不可用时可临时改用 `--mode lexical`，并在结果中明确缺失的通道。
4. 只有本地结果不足、用户要求近期外部工作或需要 DOI/题录核验时，运行同一命令并改为 `--scope all`。
