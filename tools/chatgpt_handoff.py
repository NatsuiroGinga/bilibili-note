...
FORBIDDEN_PARTS = frozenset({"runs", ".env", ".ssh", ".git", "raw"})
FORBIDDEN_SUFFIXES = (".pdf", ".pem", ".key", ".pt", ".pth", ".ckpt")
...
def initialize(args: argparse.Namespace) -> None:
    ...
    templates = {
        destination / "TEMPLATE.md": (
            "# ChatGPT 交接包模板\n\n"
            "交接包由 chatgpt_handoff.py create 生成，不手工填入凭据、数据或日志。\n"
        ),
...
def create(args: argparse.Namespace) -> None:
    ...
    if not task or len(task) > 8000 or "\x00" in task:
        raise ValueError("task 不能为空、不得含 NUL 且不得超过 8000 字符")
    ...
    rationale = validate_scalar("selection-rationale", args.selection_rationale, maximum=500)
    source_scope = validate_scalar("source-scope", args.source_scope, maximum=500)
