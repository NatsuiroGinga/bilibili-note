...
| --- | --- | --- | --- |
| 通信信道级分组、包数／到达间隔／端口行为序列、标识随机化和序列移除消融 | 首次以去身份行为序列表示加密恶意流量、首次把多类行为序列用于未知恶意检测 | 在 LSPR23 冻结无身份行为码，RWKV 只建模下一行为码正常分布；后缀状态只条件化该分布，不作为独立分类器，LSPR24 不更新码本或阈值 | 比较连续特征、CBSeq 式行为序列、VQ 行为码、下一码预测、后缀条件下一码预测；强基线含 CBSeq、PLM-NIDS、固定 n-gram、SAM Decoding 和 COMET。若行为码反预测身份或后缀条件不优于 n-gram，则否决 |
