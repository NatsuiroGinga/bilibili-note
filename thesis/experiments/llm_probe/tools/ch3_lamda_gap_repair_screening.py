...
ARMS = [
    ...
    "gap_repair_conditional_risk_gate",
]
THRESHOLD = 0.5
...
def load_config(path: Path) -> dict[str, Any]:
    ...
    if config.get("arms") != ARMS:
        raise ValueError("缺口修复筛查必须恰有四个预注册实验臂")
    if (
    ...
    if identity.get("run_tier") != "screening_only" or identity.get("published_feature_space_uses_future_covariates") is not True:
        raise ValueError("缺口修复筛查只允许 screening_only 且必须披露未来协变量")
    ...
    if mechanism.get("repair_weight") != "margin_deficit" or mechanism.get("risk_gate") != "conditional_a_gem_two_reference":
        raise ValueError("候选机制定义不匹配")
...
def weighted_repair_loss(probabilities: torch.Tensor, weights: torch.Tensor) -> torch.Tensor:
    if probabilities.numel() == 0:
        return torch.zeros((), device=probabilities.device)
    per_sample = F.binary_cross_entropy(probabilities, torch.ones_like(probabilities), reduction="none")
    normalized = weights / torch.clamp(weights.sum(), min=torch.finfo(weights.dtype).eps)
    return torch.sum(per_sample * normalized)
...
def train_epoch(
    ...
    replay_values: np.ndarray | None,
    replay_labels: np.ndarray | None,
    ...
    train_repair = repair_mask
    train_weights = repair_weights
    train_benign = benign_mask
    if replay_values is not None and replay_labels is not None and len(replay_labels):
        ...
        train_repair = np.concatenate((train_repair, np.zeros(len(replay_labels), dtype=bool)))
        train_weights = np.concatenate((train_weights, np.zeros(len(replay_labels), dtype=np.float32)))
    ...
    train_repair, train_weights, train_benign = train_repair[order], train_weights[order], train_benign[order]
    ...
    for start in range(0, len(train_labels), batch_size):
        ...
        if use_gap_repair and np.any(batch_repair):
            ...
            repair_loss = weighted_repair_loss(repair_probabilities, weights)
            loss = loss + repair_loss
        if not use_risk_gate:
            loss.backward()
            optimizer.step()
        else:
            current_grad = flat_grads(torch.autograd.grad(loss, params, retain_graph=True), params)
            protect_grad = None
            benign_grad = None
            if protect_tensor is not None:
                protect_probabilities = model(protect_tensor)
                protect_loss = criterion(protect_probabilities, torch.ones_like(protect_probabilities))
                protect_grad = flat_grads(torch.autograd.grad(protect_loss, params, retain_graph=True), params)
            if np.any(batch_benign):
                benign_inputs = inputs[torch.from_numpy(batch_benign).to(device=device)]
                benign_probabilities = model(benign_inputs)
                benign_loss = criterion(benign_probabilities, torch.zeros_like(benign_probabilities))
            ...
            assign_flat_grads(params, projected)
            optimizer.step()
            projection_events += int(event_p) + int(event_n)
            projection_dot_sum += dot_p + dot_n
    ...
    return total_loss / total_samples, total_samples, {
        "repair_loss": repair_loss_total / total_samples,
        "repair_batches": int(np.sum(repair_mask)),
        "projection_events": projection_events,
        "projection_dot_sum": projection_dot_sum,
    }
...
def checkpoint_payload(
    ...
    return {
        "schema_version": "ch3-lamda-gap-repair-screening-checkpoint-v1",
...
def execute_arm(
    ...
    use_gap_repair = arm in {"gap_repair", "gap_repair_conditional_risk_gate"}
    use_risk_gate = arm in {"conditional_risk_gate", "gap_repair_conditional_risk_gate"}
    ...
    for index in range(year_index, len(RUN_YEARS)):
        ...
        if index == year_index and next_epoch > 1 and context is not None:
            ...
            teacher = teacher_scores(model, values, device) if index > 0 else np.full(len(labels), np.nan, dtype=np.float32)
            repair_mask = (labels == 1) & np.isfinite(teacher) & (teacher < THRESHOLD)
            repair_weights = np.where(repair_mask, THRESHOLD - teacher, 0.0).astype(np.float32)
            benign_mask = (labels == 0) & np.isfinite(teacher) & (teacher >= THRESHOLD)
            context = {"year": year, "teacher": teacher, "repair_mask": repair_mask, "repair_weights": repair_weights, "benign_mask": benign_mask}
        memory_scores = teacher_scores(model, memory.values, device) if len(memory.labels) else np.empty((0,), dtype=np.float32)
        protect_values = memory.values[(memory.labels == 1) & (memory_scores >= THRESHOLD)] if len(memory.labels) else np.empty((0, len(features)), dtype=np.uint8)
        replay_values = memory.values if len(memory.labels) else None
        replay_labels = memory.labels if len(memory.labels) else None
        epoch_start = next_epoch if index == year_index else 1
        for epoch in range(epoch_start, epochs + 1):
            loss, samples, auxiliary = train_epoch(model, optimizer, criterion, values, labels, repair_mask, repair_weights, benign_mask, replay_values, replay_labels, protect_values, device, batch_size, seed + index * epochs + epoch, use_gap_repair, use_risk_gate)
            yearly.setdefault(str(year), {"epochs": []})["epochs"].append({"epoch": epoch, "loss": loss, "samples": samples, "auxiliary": auxiliary, "repair_candidates": int(np.sum(repair_mask)), "benign_candidates": int(np.sum(benign_mask)), "protect_candidates": int(len(protect_values)), "completed_at": base.now_utc()})
...
def main() -> int:
    parser = argparse.ArgumentParser(description="LAMDA 来源／开发期缺口修复与条件风险门控筛查")
    ...
    base.write_status(output_root, "running", "开始 LAMDA 缺口修复与条件风险门控筛查", started_at)
    try:
        ...
        base.write_status(output_root, "completed", "LAMDA 缺口修复与条件风险门控筛查完成", started_at)
        return 0
