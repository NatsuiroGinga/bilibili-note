...
# 四格之间**允许**不同的只有这几项；其余不一致即为疑似跑错档。
# 键名必须与 describe() 的返回键逐字一致——2026-09-04 曾因此处残留已删除的
# 「因果记忆CEM」而使工具在打完表后 KeyError 崩溃（表已打印，判断未做）。
MECHANISM_KEYS = ("实体级BCE", "尾部聚合ETA", "实体排序BER", "历史机制")
MUST_MATCH = (
