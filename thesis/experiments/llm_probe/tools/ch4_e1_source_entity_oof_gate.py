...
SCHEMA_VERSION = "ch4-e1-source-entity-oof-gate-config-v1"
CHECKPOINT_SCHEMA = "ch4-e1-source-entity-oof-selected-checkpoint-v1"
INFLIGHT_SCHEMA = "ch4-e1-source-entity-oof-inflight-v1"
RECEIPT_SCHEMA = "ch4-e1-source-entity-oof-unit-receipt-v1"
UNIT_STATUS_SCHEMA = "ch4-e1-source-entity-oof-unit-status-v1"
...
def validate_config(config: dict[str, Any]) -> None:
    ...
    if config.get("run_id") != "ch4-e1-source-entity-oof-gate-seed42-v1":
    ...
    if evaluation["saturation_threshold"] != 0.995:
        raise ValueError("饱和阈值不符")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="第四章 E1 源年实体三折折外信号快速门禁")
    ...
    class SourceMlp(nn.Module):
        def __init__(self, feature_count: int, hidden_size: int, dropout: float, aggregate: bool):
            ...
            self.encoder = nn.Sequential(
                nn.Linear(feature_count, hidden_size), nn.ReLU(), nn.Dropout(dropout)
            )
...
def build_model(config: dict[str, Any], cell: str) -> SourceMlp:
...
def source_identity(config: dict[str, Any], output_root: Path, resume: bool) -> dict[str, Any]:
    ...
    receipt = {
        "schema_version": "ch4-e1-source-input-identity-v1",
...
def train_or_resume_unit(
    ...
    resume: bool,
) -> tuple[SourceMlp, dict[str, Any], str]:
    checkpoint_path, inflight_path, receipt_path = unit_paths(output_root, cell, fold)
    identity = {**identity, "schema_version": "ch4-e1-source-unit-identity-v1"}
...
def write_status(output_root: Path, state: str, stage: str, exit_code: int | None, detail: str) -> None:
    atomic_json(
        ...
        {
            "schema_version": "ch4-e1-source-oof-status-v1",
            "state": state,
            "stage": stage,
            "detail": detail,
            "exit_code": exit_code,
...
def build_manifest(output_root: Path, run_id: str) -> None:
    ...
    atomic_json(
        ...
        {
            "schema_version": "ch4-e1-source-oof-manifest-v1",
            "run_id": run_id,
...
def load_prepared_identity(config: dict[str, Any], output_root: Path) -> dict[str, Any]:
    ...
    receipt = load_json(receipt_path)
    if (
        receipt.get("schema_version") != "ch4-e1-source-input-identity-v1"
...
def load_source_context(
    ...
    fold_receipt_value = {
        "schema_version": "ch4-e1-source-entity-fold-receipt-v1",
...
def train_unit(config: dict[str, Any], args: argparse.Namespace, config_path: Path) -> None:
    ...
    selected = validate_selected_checkpoint(
        checkpoint_path,
        receipt_path,
        {**identity, "schema_version": "ch4-e1-source-unit-identity-v1"},
    )
    if selected is not None:
        if not args.resume:
            raise RuntimeError(f"全新运行已有完成单元，拒绝覆盖：{cell}/fold{fold}")
...
def aggregate(config: dict[str, Any], args: argparse.Namespace, config_path: Path) -> None:
    ...
    threshold = config["evaluation"]["saturation_threshold"]
    if c00_pooled >= threshold and c11_pooled >= threshold:
        verdict = "saturated_close_source_entity_oof_selection"
        signal_level = "saturated"
    elif delta_pooled > fold_delta_range:
        verdict = "discriminative_allow_coarse_and_fine_representation_selection"
        signal_level = "discriminative"
    else:
        verdict = "weak_allow_coarse_rejection_only"
        signal_level = "weak"
    ...
    result = {
        "schema_version": "ch4-e1-source-entity-oof-results-v1",
        ...
        "mechanical_verdict": {
            ...
            "rule_1_both_saturated": c00_pooled >= threshold and c11_pooled >= threshold,
            "rule_2_delta_gt_fold_delta_range": delta_pooled > fold_delta_range,
    ...
    atomic_json(
        ...
        {
            "schema_version": "ch4-e1-source-oof-resume-receipt-v1",
    ...
    log(f"计算完成：C00={c00_pooled:.8f} C11={c11_pooled:.8f} verdict={verdict}")
...
def publish_aggregate(config: dict[str, Any], args: argparse.Namespace) -> None:
    ...
    atomic_json(
        ...
        {
            "schema_version": "ch4-e1-source-oof-swanlab-receipt-v1",
            "completed": True,
            "aggregate_only": True,
