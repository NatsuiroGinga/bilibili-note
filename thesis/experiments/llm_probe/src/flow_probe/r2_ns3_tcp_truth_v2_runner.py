...
PARAMS_SCHEMA: Final = "flow_probe_r2_ns3_tcp_truth_v2_params_v2"
EXPECTED_MANIFEST_SHA256: Final = (
    "d867c05013ff9c566d965ec3115bcefe6c905cca69865b042eb81f56e012a111"
)
MAIN_SCHEMA: Final = "flow_probe_r2_ns3_protocol_windows_v2"
...
def _load_manifest(path: Path) -> dict[str, ProtocolRunConfig]:
    if _file_sha256(path) != EXPECTED_MANIFEST_SHA256:
        raise TcpTruthV2Error("冻结清单 SHA-256 不一致")
    selected: dict[str, ProtocolRunConfig] = {}
    with path.open("r", encoding="ascii") as source:
        for line_number, line in enumerate(source, start=1):
            ...
            if run.transport_family == "TCP":
                if run.physics_group_sha256 in selected:
                    raise TcpTruthV2Error("同一物理组出现多个 TCP 运行")
                selected[run.physics_group_sha256] = run
    if len(selected) != 256:
        raise TcpTruthV2Error("冻结清单未恰含256个 TCP 运行")
    return selected


def _load_selection(
    closure: Mapping[str, object], manifest: Mapping[str, ProtocolRunConfig]
) -> list[tuple[str, ProtocolRunConfig]]:
    if closure.get("schema_version") != "flow_probe_r2_ns3_tcp_truth_v2_field_closure_v1":
    ...
    for item in values:
        ...
        if group in seen or group not in manifest:
            raise TcpTruthV2Error("字段闭合物理组重复或不在冻结清单")
        seen.add(group)
        result.append((coverage, manifest[group]))
...
def _run_one(
    ...
    receipt = {
        "schema_version": "flow_probe_r2_ns3_protocol_dynamics_v2_run_receipt_v1",
        "status": "pass",
        "coverage": coverage,
        "physics_group_sha256": run.physics_group_sha256,
...
def _run_from_params(path: Path) -> Mapping[str, object]:
    ...
    resume_root = _resolve_under(
        project_root, params["resume_from_output_dir"], "resume_from_output_dir"
    )
    ...
    reused = _load_reused_receipts(
        resume_root, selected, expected_hashes[contract_path]
    )
    pending = [
    ...
    state = {
        ...
        "resume_from_output_dir": resume_root.relative_to(project_root).as_posix(),
    }
    ...
    try:
        ...
        summary: dict[str, object] = {
            ...
            "reused_run_count": len(reused),
            "resume_from_output_dir": resume_root.relative_to(project_root).as_posix(),
            "sender_count_values": [8],
            "sender_compositions": ["8+0", "4+4"],
