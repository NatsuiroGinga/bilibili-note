...
def create_physics_expert(config: ExpertConfig) -> Any:
    ...
    if expert_class is None:
        ...
        raise R2FinalExpertError(
            f"未知物理专家 {config.name}；可用实现为：{available}"
        )
        ...
        class TorchExpert(torch.nn.Module):
            ...
            def _encode(self, observations: Any, valid_mask: Any) -> tuple[Any, Any]:
                ...
                projected = torch.nn.functional.gelu(
                    self.input_projection(observations)
                )
            ...
            def _physics_residual(
                ...
                del observations, encoded, state, valid_mask
                raise R2FinalExpertError(
                    f"专家 {self.config.name} 尚未定义固定物理残差"
                )

            def fixed_residual_from_state(self, state: Any) -> Any:
                """仅供运行时收据直接审计固定残差算子。"""
                ...
                return self._physics_residual(None, None, state, valid)
            ...
            def forward(
                ...
                predicted_state = torch.nn.functional.softplus(
                    self.state_head(encoded)
                )
                ...
                representation = representation * has_history.unsqueeze(1).to(
                    representation.dtype
                )
                representation = representation * active.unsqueeze(1).to(
                    representation.dtype
                )
                window_representation = window_representation * window_active.unsqueeze(
                    -1
                ).to(window_representation.dtype)
...
class SharedConservationExpert(_CausalExpertBase):
    ...
    def __new__(cls, config: ExpertConfig) -> Any:
        ...
        def shared_residual(
            ...
            balance = (
                queue_end - queue_start - arrived + served + dropped
            )
...
class TcpDynamicsExpert(_CausalExpertBase):
    ...
    def __new__(cls, config: ExpertConfig) -> Any:
        ...
        if config.required_truth_fields != expected:
            raise R2FinalExpertError("TCP 发送者级状态字段不符合 v2 冻结顺序")
        ...
        def tcp_residual(
            ...
            del observations, encoded, valid_mask
            if state.shape[-1] != 10:
            ...
            acked_bytes = state[..., 6]
            loss_gate = torch.clamp(state[..., 7], min=0.0, max=1.0)
            timeout_gate = torch.clamp(state[..., 8], min=0.0, max=1.0)
            # 所有字节状态已按 1e6 缩放；MSS 固定为场景合同中的 1448 字节。
            mss = 1448.0 / 1_000_000.0
            ack_growth = acked_bytes * mss / torch.clamp(cwnd_start, min=mss)
            ack_balance = cwnd_end - cwnd_start - ack_growth
            loss_target = torch.maximum(
                bytes_in_flight_start * 0.5,
                torch.full_like(bytes_in_flight_start, 2.0 * mss),
            )
            loss_balance = loss_gate * (ssthresh_end - loss_target)
            timeout_balance = timeout_gate * (cwnd_end - mss)
            residual = torch.stack(
                (ack_balance, loss_balance, timeout_balance), dim=-1
            )
            if self.config.residual_dimension != 3:
                raise R2FinalExpertError("TCP v2 固定残差维数必须为 3")
            return residual

        expert._physics_residual = tcp_residual.__get__(expert, type(expert))
        expert.residual_contract = "tcp_sender_newreno_window_v2"
        expert.formal_dynamics_ready = True
        ...
        class TorchUnknownFallback(torch.nn.Module):
            ...
            def forward(
                ...
                return ExpertOutput(
                    ...
                    predicted_state=zeros.expand(
                        -1, -1, config.state_dimension
                    ),
                    physics_residual=zeros.expand(
                        -1, -1, config.residual_dimension
                    ),
                    valid_mask=valid_mask,
                    active_mask=active,
    ...
    def __new__(
        ...
        *,
        ...
        class TorchUnifiedControl(torch.nn.Module):
            ...
            def forward(self, observations: Any, valid_mask: Any) -> RoutedPhysicsOutput:
                shared_output = self.shared(observations, valid_mask)
                tcp_output = self.components["tcp_shape"](observations, valid_mask)
        ...
        *,
        ...
        class TorchSystem(torch.nn.Module):
            ...
            def _effective_routes(self, route: RouteBatch) -> tuple[Any, Any, int]:
                ...
                quic_closed = quic_requested & (
                    (~quic_applicable) | (not self.quic_expert_ready)
                )
...
def fixed_residual_zero_collapse_receipt(system: Any) -> Mapping[str, object]:
    ...
    for name, (expert, values) in cases.items():
        ...
        if legacy_parameters:
            raise R2FinalExpertError(
                f"固定残差仍含旧可训练投影：{name}={legacy_parameters}"
            )
        ...
        invariance_difference = float(
            torch.max(torch.abs(residual_before - residual_after)).item()
        )
    ...
    formal_ready = all(
        bool(experts[name]["formal_dynamics_ready"])
        for name in (EXPERT_TCP, EXPERT_UDP)
    )
    return {
        "schema_version": "r2_fixed_residual_zero_collapse_receipt_v1",
        "experts": experts,
        ...
        "blocking_reason": (
            ""
            if formal_ready
            else "TCP 或 UDP 固定动力学残差未通过运行时门禁"
        ),
    }
