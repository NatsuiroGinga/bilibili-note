...
if [[ "$#" -ne 1 ]]; then
  printf '%s\n' '用法：bash scripts/run_r2_protocol_source_preflight.sh <参数JSON>' >&2
  exit 2
fi

params_path="$1"
if [[ ! -f "$params_path" || -L "$params_path" ]]; then
  printf '正式源预检参数不是普通文件：%s\n' "$params_path" >&2
  exit 2
fi
...
if [[ "$schema_version" != 'flow_probe_r2_source_preflight_params_v1' ]]; then
  printf '%s\n' '正式源预检参数模式版本不符合合同' >&2
  exit 2
fi
if [[ "$host_role" != 'local_producer' ]]; then
  printf '%s\n' '正式源预检只允许 local_producer' >&2
  exit 2
fi
...
jq -n \
  --arg schema_version 'flow_probe_r2_source_preflight_capabilities_v1' \
...
CUDA_VISIBLE_DEVICES='' PYTHONPATH="$project_root/src" \
  "$python_executable" -B -m flow_probe.r2_protocol_contract \
  preflight --params "$params_path" 2>&1 | tee "$log_path"
