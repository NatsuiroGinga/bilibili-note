...
readonly RUN_ID=ch3-full-mlp-complete-entity-lp-protocol-a-q0-seed42-v1
readonly SCREEN_NAME=ch3-full-mlp-lp-pa-q0-s42-v1
readonly CONFIG_PATH="$PROJECT_ROOT/configs/ch3-full-mlp-complete-entity-lp-protocol-a-q0-seed42-v1.json"
readonly TOOL_PATH="$PROJECT_ROOT/tools/ch3_full_mlp_complete_entity_lp_protocol_a_q0.py"
readonly SCRIPT_PATH="$PROJECT_ROOT/scripts/remote_launchers/run_ch3_full_mlp_complete_entity_lp_protocol_a_q0_seed42_v1.sh"
...
readonly PEER_RUN_PATTERN='python.*ch3_full_mlp_complete_entity_lp_protocol_a_q0[.]py'
...
valid = (
    ...
    and config["evaluation"]["p_grid"] == [0.5, 1.0, 2.0, 4.0, 8.0]
...
raise SystemExit(0 if len(names) == 6 and all((root / f"{name}.npy").is_file() for name in names) else 66)
' "$CONFIG_PATH"
}
...
value = {
    ...
    "gpu_admission_basis": "full_mlp_protocol_a_plan_12288_mib",
...
if result_path.is_file():
    ...
    result.setdefault("resource", {})["resource_measurement_contended"] = contended
...
required = ["selection_frozen.json", "source-gate-results.json", "aggregate-results.json",
...
required += [f"checkpoints/epochs/{cell}/epoch-{epoch:02d}.pt" for cell in ("B00", "B10", "O01", "O11") for epoch in range(1, 21)]
valid = (
    result["run_id"] == sys.argv[2]
    and result["source_gate"]["target_year_arrays_read"] == 0
...
worker() {
    ...
    publish_results
    validate_outputs
    launcher_status finished complete full_mlp_complete_entity_lp_protocol_a_q0_finished 0
}
...
if screen -ls 2>/dev/null | rg -q "[.]${SCREEN_NAME}[[:space:]]" \
    || pgrep -f 'python.*[c]h3_full_mlp_complete_entity_lp_protocol_a_q0.py' >/dev/null; then
    printf '同名会话或进程已运行。\n' >&2
    exit 75
fi
...
printf 'CH3_FULL_MLP_COMPLETE_ENTITY_LP_PROTOCOL_A_Q0_STARTED session=%s output=%s cells=4 cells_serial=1 resume=%s\n' "$SCREEN_NAME" "$OUTPUT_ROOT" "${resume_flag:-false}"
