...
# 直接传入 --worker。
#
# 启动前按顺序执行九道机械门禁（见 run_gates），任一门禁失败都保留诊断并以
...
readonly RUN_ID=ch3-tabm32-paper-recipe-protocol-a-seed42-v1
readonly SCREEN_NAME="$RUN_ID"
readonly CONFIG_PATH="$PROJECT_ROOT/configs/ch3-tabm32-paper-recipe-protocol-a-seed42-v1.json"
readonly TOOL_PATH="$PROJECT_ROOT/tools/ch3_tabm32_paper_recipe_protocol_a.py"
readonly SCRIPT_PATH="$PROJECT_ROOT/scripts/remote_launchers/run_ch3_tabm32_paper_recipe_protocol_a_seed42_v1.sh"
...
# 冻结配方哈希，与 tools/ch3_tabm32_paper_recipe_protocol_a.py 的
# FROZEN_RECIPE_SHA256 常量及配置文件 frozen_recipe_sha256 字段三方一致。
readonly FROZEN_RECIPE_SHA256="3b1e3103c9a8d37afe55c5e055cfcc4b84c72af3054b4cd20ec1dc4e5554d359"
readonly -a STAGE_ORDER=(select-input select-optimizer cells evaluate)
# 防止 pgrep 匹配到自身：用 [c] 字符类拆开固定脚本名首字母。
readonly PEER_PROCESS_PATTERN="python.*[c]h3_tabm32_paper_recipe_protocol_a[.]py.*${RUN_ID}"
...
value = {
    "schema_version": "ch3-tabm32-paper-recipe-protocol-a-launcher-status-v1",
...
path = pathlib.Path(sys.argv[1])
value = {
    "schema_version": "ch3-tabm32-paper-recipe-protocol-a-stage-status-v1",
...
# ---------------------------------------------------------------------------
# 九道机械门禁（1 加载 activate.sh 已在脚本顶部完成，2-9 见下）
# ---------------------------------------------------------------------------
...
# 门禁 3：冻结配方哈希比对（独立于工具内部校验的第二重核对）。
gate_frozen_hash() {
    uv run --no-sync python -c '
import json, pathlib, sys
config = json.loads(pathlib.Path(sys.argv[1]).read_text(encoding="utf-8"))
actual = config.get("frozen_recipe_sha256")
expected = sys.argv[2]
if actual != expected:
    print(f"冻结配方哈希不符：配置={actual} 期望={expected}", file=sys.stderr)
    raise SystemExit(78)
print(f"冻结配方哈希核对通过：{actual}")
' "$CONFIG_PATH" "$FROZEN_RECIPE_SHA256"
}

# 门禁 4：字段基数收据存在且 complete 为真。
gate_cardinality_receipt() {
    local config_path_field
    if ! config_path_field=$(uv run --no-sync python -c '
import json, pathlib, sys
config = json.loads(pathlib.Path(sys.argv[1]).read_text(encoding="utf-8"))
...
' "$CONFIG_PATH"); then
    ...
    if [[ "$config_path_field" != "$CARDINALITY_RECEIPT_PATH" ]]; then
        printf '配置字段基数收据路径与启动器冻结路径不一致：配置=%s 启动器=%s\n' \
            "$config_path_field" "$CARDINALITY_RECEIPT_PATH" >&2
        return 78
    fi
...
# 门禁 5：精度裁决门。配置里若 precision_pending_ruling 为真，显式失败退出；
# 当前冻结配置尚无该字段，视为未待定，门禁通过。
gate_precision_ruling() {
    local flag
    if ! flag=$(uv run --no-sync python -c '
import json, pathlib, sys
config = json.loads(pathlib.Path(sys.argv[1]).read_text(encoding="utf-8"))
print("pending" if config.get("precision_pending_ruling") is True else "ok")
' "$CONFIG_PATH"); then
        printf '读取配置字段 precision_pending_ruling 失败\n' >&2
        return 78
    fi
    if [[ "$flag" == pending ]]; then
        printf '精度合同冲突未裁决，见实施计划第二节冲突五\n' >&2
        return 70
    fi
    printf '精度裁决门通过：precision_pending_ruling 非真\n'
}

# 门禁 6：显存阈值门。resource_contract.minimum_free_gpu_memory_mib 若为
...
' "$CONFIG_PATH"); then
    ...
    if [[ "$value" == null ]]; then
        printf 'GPU 显存准入阈值待 Codex 依实施计划冲突四重新裁定：resource_contract.minimum_free_gpu_memory_mib 为 null\n' >&2
        return 71
    fi
    if [[ ! "$value" =~ ^[0-9]+$ ]]; then
...
# 门禁 7：调用 tools/memory_admission_gate.sh（读 cgroup，不读宿主机
...
# 门禁 8：磁盘可用量满足 resource_contract.minimum_free_disk_gib。
gate_disk_space() {
...
run_gates() {
    ...
    run_gate "05-precision-ruling" "$LAUNCHER_ROOT/gates/05-precision-ruling.log" gate_precision_ruling || return $?
    run_gate "06-gpu-threshold-present" "$LAUNCHER_ROOT/gates/06-gpu-threshold-present.log" gate_gpu_threshold_present || return $?
...
value = {
    "schema_version": "ch3-tabm32-paper-recipe-protocol-a-resource-receipt-v1",
    "run_id": sys.argv[2],
...
printf 'CH3_TABM32_PAPER_RECIPE_PROTOCOL_A_STARTED session=%s output=%s start_stage=%s resume=%s\n' \
