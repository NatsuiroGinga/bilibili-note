...
readonly RUN_ID=ch4-xgb-confidence-fallback-source-q0-seed42-v1
readonly TOOL_PATH="$PROJECT_ROOT/tools/ch4_xgb_confidence_fallback_source_q0.py"
readonly CONFIG_PATH="$PROJECT_ROOT/configs/ch4-xgb-confidence-fallback-source-q0-seed42-v1.json"
readonly PARENT_CONFIG_PATH="$PROJECT_ROOT/configs/ch3-xgb-cpa-elp-gpu-oof-seed42-v1.json"
readonly SCRIPT_PATH="$PROJECT_ROOT/scripts/remote_launchers/run_ch4_xgb_confidence_fallback_source_q0_seed42_v1.sh"
...
readonly PBC_RECOVERY_CONSUMER_ID=ch4-xgb-pbc-q0-seed42-v1-rerun2
readonly RECOVERY_PROOF_PATH="$PROJECT_ROOT/runs/recovery/${PARENT_RUN_ID}-for-${PBC_RECOVERY_CONSUMER_ID}-v1/parent-recovery-proof.json"
...
write_status() {
    ...
    printf '{"run_id":"%s","parent_run_id":"%s","screening_only":true,"formal":false,"source_year_only":true,"target_year_arrays_read":0,"new_models_trained":0,"state":"%s","stage":"%s","detail":"%s","exit_code":%s}\n' \
        "$RUN_ID" "$PARENT_RUN_ID" "$state" "$stage" "$detail" "$exit_code" > "$partial"
...
write_resource_receipt() {
    ...
    printf '{"schema_version":"ch4-xgb-confidence-fallback-source-q0-resource-v1","estimated_peak_gib":%s,"cgroup_memory_max_bytes":%s,"cgroup_memory_current_bytes":%s,"cgroup_inactive_file_bytes":%s,"cgroup_effective_available_gib":%s,"disk_available_mib":%s,"gpu_free_mib":%s,"memory_admission_exit":%s,"source_year_only":true,"passed":%s}\n' \
...
tracking_gate() {
    ...
    printf '{"schema_version":"ch4-xgb-confidence-fallback-source-q0-swanlab-gate-v1","workspace":"mortiswang","project":"ns3-rwkv-lspr24","aggregate_only":true,"ping_exit":%s,"verify_exit":%s,"passed":%s}\n' \
...
worker() {
    ...
    if [[ "$python_exit" -ne 0 ]]; then
        write_status failed source_q0 python_failed "$python_exit"
        return "$python_exit"
    fi
    if [[ "$tee_exit" -ne 0 ]]; then
        write_status failed source_q0 tee_failed "$tee_exit"
        return "$tee_exit"
    fi
    for name in resource-receipt.json source-threshold-seal.json target-candidate-seal.json aggregate-results.json swanlab-receipt.json manifest.json; do
        if [[ ! -s "$RUN_ROOT/$name" ]]; then
            write_status failed postcondition required_aggregate_artifact_missing 16
            return 16
        fi
    done
...
for path in "$TOOL_PATH" "$CONFIG_PATH" "$PARENT_CONFIG_PATH" "$SCRIPT_PATH" "$RECOVERY_PROOF_PATH"; do
    [[ -s "$path" ]] || {
        printf '生产文件缺失：%s\n' "$path" >&2
        exit 67
    }
done
for name in status.json selection_frozen_xgb2x2.json effective_config_receipts.json model_oof_semantic168_fold0.json model_oof_semantic168_fold1.json model_oof_semantic168_fold2.json; do
    [[ -r "$PARENT_RUN_ROOT/$name" && -s "$PARENT_RUN_ROOT/$name" ]] || {
        printf '父运行制品缺失或不可读：%s\n' "$PARENT_RUN_ROOT/$name" >&2
        exit 66
    }
done
for cache_name in X23 y23 E23 I23 M23; do
...
if pgrep -f 'python.*[c]h4_xgb_confidence_fallback_source_q0.py' >/dev/null; then
    printf '同名源年 Q0 进程已经运行。\n' >&2
    exit 75
fi
...
sha256sum "$TOOL_PATH" "$CONFIG_PATH" "$PARENT_CONFIG_PATH" "$SCRIPT_PATH" \
    ...
    "$PARENT_RUN_ROOT/model_oof_semantic168_fold2.json" > "$LAUNCH_ROOT/input-sha256.log"
...
printf 'CH4_XGB_CONFIDENCE_FALLBACK_SOURCE_Q0_STARTED run=%s pid=%s\n' "$RUN_ROOT" "$!"
