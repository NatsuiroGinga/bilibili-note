...
readonly RUN_ID=ch3-protocol-a-raw83-target-v1
readonly SCREEN_NAME=ch3-protocol-a-raw83-target-v1
readonly CONFIG_PATH="$PROJECT_ROOT/configs/ch3-protocol-a-raw83-target-v1.json"
readonly TOOL_PATH="$PROJECT_ROOT/tools/ch3_protocol_a_raw83_prepare.py"
readonly SCRIPT_PATH="$PROJECT_ROOT/scripts/remote_launchers/run_ch3_protocol_a_raw83_target_v1.sh"
...
value = {
    "schema_version": "ch3-protocol-a-raw83-target-launcher-status-v1",
...
validate_static_contract() {
    for path in "$CONFIG_PATH" "$TOOL_PATH" "$SCRIPT_PATH" \
        ...
        "$PROJECT_ROOT/src/flow_probe/protocol_a_preprocessing.py"; do
        [[ -s "$path" && ! -L "$path" ]] || {
...
valid = (
    value.get("qualification_status") == "qualified-source-arm-sealed"
    and value.get("winning_arm") in {"A", "B"}
    and isinstance(value.get("source_product", {}).get("qualification_seal_sha256"), str)
    and len(value["source_product"]["qualification_seal_sha256"]) == 64
)
...
admit_resources() {
    local plan required_bytes required_kib disk_fields available_kib used_percent maximum_used
    ...
    if [[ ! "$available_kib" =~ ^[0-9]+$ || ! "$used_percent" =~ ^[0-9]+$ ]] \
        ...
        printf 'P6 磁盘资源门失败：available_kib=%s required_kib=%s used_percent=%s\n' \
            "$available_kib" "$required_kib" "$used_percent" >&2
        return 69
    fi
}
...
resource_monitor() {
    ...
    while true; do
        ...
        if [[ -r /sys/fs/cgroup/memory.current ]]; then current=$(< /sys/fs/cgroup/memory.current); fi
        disk_fields=$(df -Pk "$PROJECT_ROOT" | tail -n 1)
        available=$(printf '%s\n' "$disk_fields" | awk '{print $4}')
        used=$(printf '%s\n' "$disk_fields" | awk '{gsub(/%/, "", $5); print $5}')
...
worker() {
    ...
    flock -n 9 || return 75
    trap '[[ -n ${monitor_pid:-} ]] && kill "$monitor_pid" 2>/dev/null || true; launcher_status interrupted p6 signal 130; exit 130' HUP INT TERM
    validate_static_contract
    admit_qualification
    admit_resources
    launcher_status running p6 sealed-year-product-started null
    resource_monitor & monitor_pid=$!
    if run_logged "$LOG_PATH" uv run --no-sync python "$TOOL_PATH" --config "$CONFIG_PATH" --through-stage P6; then code=0; else code=$?; fi
    kill "$monitor_pid" 2>/dev/null || true
    wait "$monitor_pid" 2>/dev/null || true
    monitor_pid=
    if ((code != 0)); then launcher_status failed p6 tool-exited "$code"; return "$code"; fi
    uv run --no-sync python "$TOOL_PATH" --config "$CONFIG_PATH" --validate-product \
        > "$OUTPUT_ROOT/receipts/launcher-product-validation.json"
    launcher_status finished p6 sealed-year-product-finished 0
}

if [[ ${1:-} == --help ]]; then usage; exit 0; fi
if [[ ${1:-} == --worker ]]; then
    [[ $# -eq 1 ]] || { usage >&2; exit 64; }
    cd "$PROJECT_ROOT"; source tools/env/activate.sh; worker; exit $?
fi
[[ $# -eq 0 ]] || { usage >&2; exit 64; }
cd "$PROJECT_ROOT"
source tools/env/activate.sh
for command in awk df flock rg screen tee uv; do
...
if screen -ls 2>/dev/null | rg -q "[.]${SCREEN_NAME}[[:space:]]" \
    || pgrep -f 'python.*[c]h3_protocol_a_raw83_prepare.py.*target-v1' >/dev/null; then
    printf '同名 P6 会话或进程已运行。\n' >&2
    exit 75
fi
mkdir -p -- "$LAUNCHER_ROOT" "$OUTPUT_ROOT"
printf '%s\n' "$SCREEN_NAME" > "$LAUNCHER_ROOT/screen-session.txt"
screen -dmS "$SCREEN_NAME" bash "$SCRIPT_PATH" --worker
printf '已启动封印后 P6：screen -r %s\n' "$SCREEN_NAME"
