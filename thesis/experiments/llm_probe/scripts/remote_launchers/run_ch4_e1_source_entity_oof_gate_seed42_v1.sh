...
readonly RUN_ID=ch4-e1-source-entity-oof-gate-seed42-v1
readonly SCREEN_NAME=ch4-e1-source-oof-s42-v1
...
readonly CONFIG_PATH="$PROJECT_ROOT/configs/ch4-e1-source-entity-oof-gate-seed42-v1.json"
readonly TOOL_PATH="$PROJECT_ROOT/tools/ch4_e1_source_entity_oof_gate.py"
readonly SCRIPT_PATH="$PROJECT_ROOT/scripts/remote_launchers/run_ch4_e1_source_entity_oof_gate_seed42_v1.sh"
...
value = {
    "schema_version": "ch4-e1-source-oof-launcher-status-v1",
...
raise SystemExit(0 if len(required) == 5 and all(path.is_file() for path in required) else 66)
' "$CONFIG_PATH"
}
...
path = pathlib.Path(sys.argv[1])
value = {
    "schema_version": "ch4-e1-source-oof-resource-receipt-v1",
...
required = [
    ...
    "resume-receipt.json", "fold-results.json", "aggregate-results.json",
...
if pgrep -f 'python.*[c]h4_e1_source_entity_oof_gate.py' >/dev/null; then
    printf '同名门禁进程已运行。\n' >&2
    exit 75
fi
...
printf 'CH4_E1_SOURCE_OOF_STARTED session=%s output=%s units=6 adaptive_parallelism=1_or_2 serial_gate=10GiB/40GiB parallel_gate=24GiB/65GiB disk_free_gate_gib=10\n' \
