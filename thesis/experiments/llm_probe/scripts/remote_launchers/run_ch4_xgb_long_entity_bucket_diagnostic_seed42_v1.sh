...
readonly RUN_ID=ch4-xgb-long-entity-bucket-diagnostic-seed42-v1
readonly TOOL_PATH="$PROJECT_ROOT/tools/ch4_xgb_long_entity_bucket_diagnostic.py"
readonly CONFIG_PATH="$PROJECT_ROOT/configs/ch3-xgb-cpa-elp-gpu-oof-seed42-v1.json"
readonly SCRIPT_PATH="$PROJECT_ROOT/scripts/remote_launchers/run_ch4_xgb_long_entity_bucket_diagnostic_seed42_v1.sh"
...
readonly ESTIMATED_PEAK_GIB=52

write_status() {
    local state=$1 stage=$2 detail=$3 exit_code=$4
    local partial="$STATUS_PATH.partial.$$"
    printf '{"run_id":"%s","parent_run_id":"%s","parent_eval_id":"%s","state":"%s","stage":"%s","detail":"%s","exit_code":%s}\n' \
...
write_resource_receipt() {
    ...
    printf '{"schema_version":"ch4-xgb-long-entity-resource-v1","estimated_peak_gib":%s,"cgroup_memory_max_bytes":%s,"cgroup_memory_current_bytes":%s,"cgroup_available_gib":%s,"disk_available_mib":%s,"gpu_free_mib":%s,"memory_admission_exit":%s,"passed":%s}\n' \
...
tracking_gate() {
    ...
    verify_exit=$?
    set -e
    if [[ "$ping_exit" -eq 0 && "$verify_exit" -eq 0 ]]; then
        passed=true
    else
        passed=false
    fi
    printf '{"schema_version":"ch4-xgb-long-entity-swanlab-gate-v1","workspace":"mortiswang","project":"ns3-rwkv-lspr24","aggregate_only":true,"ping_exit":%s,"verify_exit":%s,"passed":%s}\n' \
...
worker() {
    ...
    nvidia-smi --query-compute-apps=pid,used_memory --format=csv,noheader \
        > "$RUN_ROOT/gpu-processes-at-launch.txt" || true
    resource_gate
    tracking_gate
    uv run --no-sync python "$TOOL_PATH" \
        ...
        --out "$RUN_ROOT" \
        --validate-inputs
    write_status running diagnostic started null
    set +e
    uv run --no-sync python "$TOOL_PATH" \
        --parent-run-root "$PARENT_RUN_ROOT" \
        --parent-eval-root "$PARENT_EVAL_ROOT" \
    ...
    printf '%s\n' "$python_exit" > "$RUN_ROOT/python-exit-code.txt"
    printf '%s\n' "$tee_exit" > "$RUN_ROOT/tee-exit-code.txt"
    if [[ "$python_exit" -ne 0 ]]; then
        write_status failed diagnostic python_failed "$python_exit"
        return "$python_exit"
    fi
    if [[ "$tee_exit" -ne 0 ]]; then
        write_status failed diagnostic tee_failed "$tee_exit"
        return "$tee_exit"
    fi
    for name in xgb_long_entity_bucket_results.json manifest.json resource-receipt.json swanlab-receipt.json; do
...
if pgrep -f 'python.*[c]h4_xgb_long_entity_bucket_diagnostic.py' >/dev/null; then
    printf '同名诊断进程已经运行。\n' >&2
    exit 75
fi
...
sha256sum "$TOOL_PATH" "$CONFIG_PATH" "$SCRIPT_PATH" \
    ...
    "$PARENT_EVAL_ROOT/manifest.json" > "$LAUNCH_ROOT/input-sha256.txt"
...
printf 'XGB_LONG_ENTITY_BUCKET_DIAGNOSTIC_STARTED run=%s pid=%s\n' "$RUN_ROOT" "$!"
