...
readonly RUN_ID=ch3-resmlp2-cpa-elp-protocol-a-2x2-seed42-v1
readonly SCREEN_NAME=ch3-resmlp2-protocol-a-s42-v1
readonly CONFIG_PATH="$PROJECT_ROOT/configs/ch3-resmlp2-protocol-a-2x2-seed42-v1.json"
readonly TOOL_PATH="$PROJECT_ROOT/tools/ch3_resmlp2_tabm_protocol_a_2x2.py"
readonly SCRIPT_PATH="$PROJECT_ROOT/scripts/remote_launchers/run_ch3_resmlp2_protocol_a_2x2_seed42_v1.sh"
...
readonly PEER_RUN_PATTERN='python.*ch3_resmlp2_tabm_protocol_a_2x2[.]py.*ch3-tabm4-protocol-a'
readonly GPU_FREE_MIN_MIB=8244
...
value = {
    ...
    "updated_at_unix": time.time(), "target_previously_accessed": True,
...
valid = (
    ...
    and config["candidate"]["residual_affine_layers"] == 2
...
path = pathlib.Path(sys.argv[1])
value = {
    "schema_version": "ch3-protocol-a-resource-receipt-v1",
    ...
    "gpu_admission_basis": "observed_tabm_single_job_peak_8244_mib",
    "resource_measurement_contended": False,
    "fair_efficiency_evidence": True,
}
temporary = path.with_name(path.name + f".partial.{os.getpid()}")
temporary.write_text(json.dumps(value, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
os.replace(temporary, path)
...
resource_monitor() {
    ...
    while true; do
        local gpu_used cgroup_current peer_job_running=0
        ...
        pgrep -f "$PEER_RUN_PATTERN" >/dev/null 2>&1 && peer_job_running=1
...
if result_path.is_file():
    ...
    result["resource"]["resource_measurement_contended"] = contended
    result["resource"]["fair_efficiency_evidence"] = not contended
...
required = ["selection_frozen.json", "aggregate-results.json", "complete-alert-budget-curves.npz",
            "complete-alert-budget-curves-receipt.json", "resource-receipt.json", "swanlab-receipt.json", "manifest.json", "status.json"]
required += [f"checkpoints/selected-{cell}.pt" for cell in ("C00", "C01", "C10", "C11")]
valid = (
    result["model"]["model_key"] == "resmlp2" and result["model"]["parameter_count"] == 89796
    and result["isolation"]["target_disk_loads"] == 1 and result["isolation"]["target_evaluation_calls"] == 4
...
worker() {
    ...
    admit_resources
    launcher_status running protocol-a source_selection_started null
    resource_monitor &
    monitor_pid=$!
...
if screen -ls 2>/dev/null | rg -q "[.]${SCREEN_NAME}[[:space:]]" \
    || pgrep -f 'python.*[c]h3_resmlp2_tabm_protocol_a_2x2.py.*ch3-resmlp2-protocol-a' >/dev/null; then
    printf '同名会话或进程已运行。\n' >&2
    exit 75
fi
...
printf 'CH3_RESMLP2_PROTOCOL_A_2X2_STARTED session=%s output=%s cells=4 cells_serial=1 resume=%s\n' "$SCREEN_NAME" "$OUTPUT_ROOT" "${resume_flag:-false}"
