...
readonly RUN_ID=ch3-xgb-cpa-elp-gpu-oof-seed42-v1-rerun1
readonly SCREEN_NAME=ch3-xgb-cpa-elp-gpu-s42-v1-rerun1
readonly TOOL_PATH="$PROJECT_ROOT/tools/ch3_xgb_cpa_elp_entity_oof.py"
readonly CONFIG_PATH="$PROJECT_ROOT/configs/ch3-xgb-cpa-elp-gpu-oof-seed42-v1.json"
readonly SCRIPT_PATH="$PROJECT_ROOT/scripts/remote_launchers/run_ch3_xgb_cpa_elp_gpu_oof_seed42_v1.sh"
...
worker() {
    ...
    write_status running source_selection started null
    set +e
    ...
    if [[ "$python_code" -ne 0 ]]; then
        write_status failed experiment python_failed "$python_code"
        return "$python_code"
    fi
    if [[ "$tee_code" -ne 0 ]]; then
        write_status failed experiment tee_failed "$tee_code"
        return "$tee_code"
    fi
...
if pgrep -f 'python.*[c]h3_xgb_cpa_elp_entity_oof.py' >/dev/null; then
    printf '同名实验进程已经运行。\n' >&2
    exit 75
fi
...
uv run --no-sync python "$TOOL_PATH" --config "$CONFIG_PATH" --validate-config
sha256sum "$TOOL_PATH" "$CONFIG_PATH" "$SCRIPT_PATH" > "$LAUNCH_ROOT/input-sha256.txt"
...
printf 'XGBOOST_CPA_ELP_STARTED session=%s run=%s\n' "$SCREEN_NAME" "$RUN_ROOT"
