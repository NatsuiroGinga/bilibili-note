...
CONFIG="configs/ch3-full-mlp-valid-2x2-source-s0-seed42-v1.json"
TOOL="tools/ch3_full_mlp_valid_2x2_source_s0.py"
RUN_ID="ch3-full-mlp-valid-2x2-source-s0-seed42-v1"
SCREEN_NAME="ch3-mlp-2x2-s0"
...
if [[ -f "${OUTPUT_ROOT}/manifest.json" ]] && [[ -f "${OUTPUT_ROOT}/status.json" ]]; then
  if rg -q '"state": "finished"' "${OUTPUT_ROOT}/status.json"; then
    printf '运行已完成：%s\n' "${RUN_ID}"
    exit 0
  fi
fi
