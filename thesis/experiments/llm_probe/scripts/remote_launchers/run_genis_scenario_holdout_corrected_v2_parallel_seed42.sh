...
readonly RUN_ID=genis-60s-same-source-stable-key-corrected-v2-parallel-baselines-seed42
readonly SCREEN_NAME=genis-corrected-v2-parallel-s42
...
readonly CONFIG_PATH="$PROJECT_ROOT/configs/genis-scenario-holdout-corrected-v2-parallel-seed42.json"
readonly MODULE_PATH="$PROJECT_ROOT/src/flow_probe/genis_scenario_holdout_corrected_v2.py"
readonly SCRIPT_PATH="$PROJECT_ROOT/scripts/remote_launchers/run_genis_scenario_holdout_corrected_v2_parallel_seed42.sh"
readonly MODULE=flow_probe.genis_scenario_holdout_corrected_v2
...
write_status() {
    ...
    printf '  "contract_version": "corrected-v2",\n' >> "$partial"
...
worker() {
    ...
    flock -n 9 || return 75
    write_status running parallel_matrix started null
    set +e
    ...
    if [[ "${matrix_status[0]}" -ne 0 || "${matrix_status[1]}" -ne 0 ]]; then
        ...
        write_status failed parallel_matrix command_or_log_failed "$matrix_exit"
        return "$matrix_exit"
    fi
    set +e
    ...
    write_status finished complete parallel_matrix_and_upload_finished 0
}
...
if [[ $# -ne 0 || -e "$OUTPUT_ROOT" || -e "$LAUNCHER_ROOT" ]]; then
    printf '参数错误，或 corrected-v2 唯一运行根/启动器根已存在。\n' >&2
    exit 73
fi
if [[ ! -s "$CONFIG_PATH" || ! -s "$MODULE_PATH" || ! -s "$SCRIPT_PATH" || ! -s "$ARCHIVE" ]]; then
    printf 'corrected-v2 生产文件或 3-scenarios.zip 不完整。\n' >&2
    exit 66
fi
...
if screen -ls 2>/dev/null | rg -q "[.]${SCREEN_NAME}[[:space:]]"; then
    printf '同名 corrected-v2 screen 已运行。\n' >&2
    exit 75
fi
if pgrep -f "flow_probe[.]genis_scenario_holdout_corrected_v2|${RUN_ID}" >/dev/null; then
    printf 'corrected-v2 同模块或同运行身份进程已存在。\n' >&2
    exit 75
fi
mkdir -p -- "$LAUNCHER_ROOT"
...
sha256sum "$CONFIG_PATH" "$MODULE_PATH" "$SCRIPT_PATH" "$ARCHIVE" \
    > "$LAUNCHER_ROOT/input-sha256.txt"
write_status prepared launch corrected_v2_parallel_contract_passed null
screen -dmS "$SCREEN_NAME" bash "$SCRIPT_PATH" --worker
printf 'GENIS_CORRECTED_V2_PARALLEL_STARTED session=%s output=%s max_concurrency=3\n' \
