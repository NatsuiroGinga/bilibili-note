...
tracking_gate() {
    ...
    printf '{"schema_version":"ch4-xgb-pbc-q0-swanlab-gate-v1","workspace":"mortiswang","project":"ns3-rwkv-lspr24","aggregate_only":true,"ping_exit":%s,"verify_exit":%s,"passed":%s}\n' \
...
worker() {
    ...
    write_status running pbc started null
    set +e
    ...
    if [[ "$python_exit" -ne 0 ]]; then
        write_status failed pbc python_failed "$python_exit"
        return "$python_exit"
    fi
    if [[ "$tee_exit" -ne 0 ]]; then
        write_status failed pbc tee_failed "$tee_exit"
        return "$tee_exit"
    fi
    for name in selection-seal.json pbc-results.json manifest.json resource-receipt.json swanlab-receipt.json; do
...
if pgrep -f 'python.*[c]h4_xgb_pbc_q0.py' >/dev/null; then
    printf '同名 PBC 进程已经运行。\n' >&2
    exit 75
fi
...
printf 'CH4_XGB_PBC_Q0_STARTED run=%s pid=%s\n' "$RUN_ROOT" "$!"
