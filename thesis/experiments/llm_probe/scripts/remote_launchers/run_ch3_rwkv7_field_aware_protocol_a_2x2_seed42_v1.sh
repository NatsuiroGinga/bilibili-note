...
value = {
    "schema_version": "ch3-rwkv7-field-aware-launcher-status-v1",
...
valid = (
    ...
    and config["adapter_order"] == ["R0", "R1", "R2"]
    and config["capacity_order"] == ["K0", "K1", "K2"]
...
path = pathlib.Path(sys.argv[1])
value = {
    "schema_version": "ch3-rwkv7-field-aware-resource-receipt-v1",
...
worker() {
    ...
    launcher_status finished complete protocol_a_finished 0
}
...
if screen -ls 2>/dev/null | rg -q "[.]${SCREEN_NAME}[[:space:]]" \
    || pgrep -f 'python.*[c]h3_rwkv7_field_aware_protocol_a_2x2.py' >/dev/null; then
    printf '同名会话或进程已运行。\n' >&2
    exit 75
fi
...
printf 'CH3_RWKV7_FIELD_AWARE_PROTOCOL_A_STARTED session=%s output=%s resume=%s\n' "$SCREEN_NAME" "$OUTPUT_ROOT" "${resume_flag:-false}"
