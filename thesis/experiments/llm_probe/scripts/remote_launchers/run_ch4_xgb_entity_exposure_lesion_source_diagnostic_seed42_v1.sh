...
readonly RUN_ID=ch4-xgb-entity-repeated-exposure-pathology-lspr23-q0-seed42-v1-rerun1
readonly SCREEN_NAME=ch4-xgb-entity-exposure-lesion-s42-v1-rerun1
readonly TOOL_PATH="$PROJECT_ROOT/tools/ch4_xgb_entity_exposure_lesion_source_diagnostic.py"
readonly CONFIG_PATH="$PROJECT_ROOT/configs/ch4-xgb-entity-exposure-lesion-source-diagnostic-seed42-v1.json"
readonly SCRIPT_PATH="$PROJECT_ROOT/scripts/remote_launchers/run_ch4_xgb_entity_exposure_lesion_source_diagnostic_seed42_v1.sh"
readonly PARENT_ROOT="$PROJECT_ROOT/runs/diagnostics/ch3-xgb-cpa-elp-gpu-oof-seed42-v1-rerun1"
readonly THRESHOLD_PATH="$PROJECT_ROOT/runs/candidates/ch4-xgb-confidence-fallback-source-q0-seed42-v1/source-threshold-seal.json"
readonly THRESHOLD_SHA256=f70cb08c5e40413934b9a812debf98da47d6034940196348a808f8377dbe9334
...
resource_gate() {
    ...
    if ! awk -v a="$available_gib" 'BEGIN {exit !(a >= 64.0)}'; then
...
if pgrep -f 'python.*[c]h4_xgb_entity_exposure_lesion_source_diagnostic.py' >/dev/null; then
    printf '同名诊断进程已经运行。\n' >&2
    exit 75
fi
...
uv run --no-sync python "$TOOL_PATH" --config "$CONFIG_PATH" --validate-config
sha256sum "$TOOL_PATH" "$CONFIG_PATH" "$SCRIPT_PATH" "$THRESHOLD_PATH" > "$LAUNCH_ROOT/input-sha256.txt"
...
printf 'CH4_ENTITY_EXPOSURE_LESION_DIAGNOSTIC_STARTED session=%s run=%s\n' "$SCREEN_NAME" "$RUN_ROOT"
