...
def build_specs(config: dict[str, Any], phase: str) -> list[ConnectionSpec]:
    ...
    combinations = itertools.product(
        implementations, profiles, seeds, config["payloads"]
    )
    return [
        ConnectionSpec(index=index, implementation=implementation, profile=profile, seed=seed, payload=payload)
...
def fixed_endpoint_contract(
    config: dict[str, Any], spec: ConnectionSpec
) -> dict[str, Any]:
...
def assert_udp_ports_exclusively_bindable(
    ...
    endpoints = [
        ...
        {"role": "proxy_listen", "endpoint": endpoint_pair(contract, "client_to_proxy", "destination")},
        {"role": "proxy_upstream", "endpoint": endpoint_pair(contract, "proxy_to_server", "source")},
        {"role": "reference_server", "endpoint": endpoint_pair(contract, "proxy_to_server", "destination")},
    ]
...
def client_command(
    *, spec: ConnectionSpec, tools_root: Path, qlog_dir: Path, response_dir: Path,
    server_ip: str, port: int, local_port: int, payload_name: str,
    ...
    if spec.implementation == "aioquic":
        ...
        command = [
            ...
            "--insecure", "--local-port", str(local_port),
            "--quic-log", str(qlog_dir), "--output-dir", str(response_dir), url,
        ]
    else:
        client = tools_root / "sources/quiche-0.24.5/target/release/quiche-client"
        environment["QLOGDIR"] = str(qlog_dir)
        command = [
            str(client), "--no-verify", "--dump-responses", str(response_dir), url,
        ]
    return command, environment


def collect_connection(
    *, project_root: Path, spec: ConnectionSpec, config: dict[str, Any],
    collection_config_sha256: str, tools_root: Path, output_root: Path,
    payload_root: Path, certificate: Path, private_key: Path, server_binary: Path,
) -> dict[str, Any]:
    ...
    client_timeout_seconds = float(config.get("client_timeout_seconds", 240))
    ...
    queue_limit = config["network_contract"]["queue_limit_packets"]
    direction_seeds = {
    ...
    if isinstance(deterministic, dict):
        expected_direction_seeds = {
            name: int(value)
            for name, value in deterministic["direction_seeds"].items()
        }
    ...
    atomic_json(
        impairment_path,
        {
            ...
            "rate_model": "per_direction_byte_serialization",
    ...
    try:
        ...
        try:
            ...
            if server_process.poll() is not None:
                raise RuntimeError(
                    f"单连接参考服务端启动失败：{server_process.returncode}"
                )

            proxy_command = [
                str(proxy_python), str(proxy_program),
                "--listen-host", str(proxy_endpoint[0]),
                "--listen-port", str(proxy_endpoint[1]),
                "--upstream-bind-host", str(upstream_endpoint[0]),
                "--upstream-bind-port", str(upstream_endpoint[1]),
                "--upstream-host", str(server_endpoint[0]),
                "--upstream-port", str(server_endpoint[1]),
                "--expected-client-host", str(client_endpoint[0]),
                "--expected-client-port", str(client_endpoint[1]),
                "--schedule-mode", schedule_mode,
                "--delay-ms", str(profile["delay_ms"]),
                "--jitter-ms", str(profile["jitter_ms"]),
                "--loss-percent", str(profile["loss_percent"]),
                "--rate-mbit", str(profile["rate_mbit"]),
                "--queue-limit-packets", str(queue_limit),
                "--client-to-server-seed", str(direction_seeds["client_to_server"]),
                "--server-to-client-seed", str(direction_seeds["server_to_client"]),
                "--config-sha256", impairment_config_sha256,
                "--ready-file", str(proxy_ready_path),
                "--stats-file", str(proxy_stats_path),
                "--event-log", str(proxy_event_path),
            ]
            ...
            endpoint_contract["observed_proxy_listen_endpoint"] = proxy_ready[
                "listen_endpoint"
            ]
            ...
            with capture_log.open("ab") as capture_stream:
                capture_process = subprocess.Popen(
                    [
                        str(tcpdump), "-Z", "root", "-i", "lo", "-s", "0",
                        "-U", "-w", str(pcap_path), "udp", "port",
            ...
            if capture_process.poll() is not None:
                raise RuntimeError(
                    f"loopback 抓包进程提前退出：{capture_process.returncode}"
                )
            ...
            with client_log.open("ab") as client_stream:
                ...
                atomic_json(endpoint_path, endpoint_contract)
                client_exit = client_process.wait(timeout=client_timeout_seconds)
            ...
            try:
                proxy_exit = stop_process(
                    proxy_process, signal_number=signal.SIGINT, timeout=30
                )
            except BaseException as error:
            ...
            try:
                server_raw_exit = stop_process(
                    server_process, signal_number=signal.SIGTERM, timeout=30
                )
                server_exit = (
                    0
                    if server_raw_exit in (0, -signal.SIGTERM, 128 + signal.SIGTERM)
                    else server_raw_exit
                )
            except BaseException as error:
            ...
            if proxy_stats_path.is_file():
                try:
                    observed_proxy_stats = json.loads(
                        proxy_stats_path.read_text(encoding="utf-8")
                    )
                    endpoint_contract["observed_client_endpoint"] = (
                        observed_proxy_stats.get("observed_client_endpoint")
                    )
                except (OSError, json.JSONDecodeError) as error:
                    cleanup_errors.append(
                        f"proxy_stats:{type(error).__name__}:{error}"
                    )
            try:
                atomic_json(endpoint_path, endpoint_contract)
            except BaseException as error:
            ...
            atomic_json(
                exit_path,
                {
                    ...
                    "server_exit_normalization": "controlled_sigterm_is_zero",
                },
            )
        ...
        qlogs = sorted(path for path in qlog_dir.iterdir() if path.is_file() and path.stat().st_size > 0)
        if not qlogs:
...
def main() -> int:
    ...
    deterministic = config.get("determinism_contract")
    if isinstance(deterministic, dict):
        target_index = int(deterministic["target_connection_index"])
        target = matrix_specs[target_index - 1]
        frozen_target = (
            target.index,
        ...
        expected_target = (
            12, "aioquic", "p03", 50, 0.0, 0.0, 20, 43, "bulk", 67108864
        )
        if frozen_target != expected_target:
            raise RuntimeError(
                f"确定性修复目标不再是冻结矩阵序号 12：{frozen_target}"
            )
        if (
            args.phase != "field"
            or args.only_index != target_index
            or float(config.get("client_timeout_seconds", 0)) != 240
            or int(config["network_contract"]["queue_limit_packets"]) != 1000
        ):
            raise RuntimeError("确定性修复配置只允许 field/only-index 12/240 秒/1000 包")
    ...
    if not certificate.is_file() or not private_key.is_file():
        run_checked(
            [
                ...
                "-subj", "/CN=r2-quic-reference.invalid",
            ]
        )
