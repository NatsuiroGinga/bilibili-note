...
Supports: 正实体池 239 个、片段数均值 54.498（截断后 6.126）；
        ...
        **训练区**负实体池由收据 effective_budgets 反解为 121,336（六档一致），
        Ŝ_n 刷新费须用此口径而非全 E23 口径
...
### 5.1 谱系对 EAS-full 的独立不利证据

凸分支能做到「求解前判定」的**唯一**原因是有对偶可圈定最优解的可行域。
深度网络没有对偶，故最近的先例（Schultheis）只能退到前向之后，并自陈仍需完整前向。
**谱系中不存在绕开对偶的「前向前证书」先例——EAS-full 没有可借用的技术路线。**
这与审核报告维度三 4.3 的预算证伪方向一致而证据独立（一条来自文献缺位，一条来自实测预算）。

### 5.2 一条反向警示：不得声称「精确优于近似」

meProp 的准确率不降反升（类 dropout 的正则效应）。
故 EAS 相对启发式方法的卖点只能落在「无需精度论证的工程确定性」，
**不能落在「效果更好」**。被审方案 §3.1 的「结构性优势」须限定在正确性论证成本的意义上。

### 5.3 待办：一条须处理的引用风险

`arXiv:1605.04337`（Narasimhan & Agarwal，pAUC 凸代理 ＋ 割平面）是 BER 目标的凸祖先，
其割平面法维护约束活动集。它不是安全筛除，但若正文声称「排序损失上无先例」，
可能被用来反驳。**须取全文确认割平面活动集的语义**；当前 `在线摘要` 级，
不得用于支撑或否定任何论断。

### 5.4 检索边界（不得越界表述）

「未找到排序／成对损失的安全筛除工作」的准确表述是
**「arXiv 题名与摘要一级未见」**——该检索不含全文、不覆盖 arXiv 之外的库。
**不得写成「文献中不存在」。**
