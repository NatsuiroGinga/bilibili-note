...
**当前状态：** 研究中。除标记为“实验支持”的条目外，其余均不能写成 LAMDA 方法有效性结论。
...
- 本项目筛选覆盖层：来源年 `2013/2014`，项目开发层 `2016/2017`，封印年 `2018--2025`。项目开发层只是为避免在 FAR 上选择方法而增加的内部筛选用途，不能写成原论文的数据角色。
- 当前公开 `4561` 维特征空间包含全时期训练分片协变量，所有现阶段候选运行均标为 `screening_only`。封印年不得参与候选、阈值或超参数选择。
...
| L11 | 双玩家安全约束回放 | L3＋L10 的博弈化组合 | 安全玩家只控制良性 FPR 约束和旧恶意回归，非攻击者—防守者叙事 | ER、L3、双玩家层、联合 | `conditional_candidate` | [博弈候选方案](第三章-LAMDA双玩家安全约束博弈候选方案.md)；[资料库源年筛选审查回收](../../.Codex/docs/chatgpt-handoffs/inbox/LAMDA-2026-源年筛选审查-资料库回收.md)；L3/L4 已完成但未通过，当前条件是先核验 TFCO/Cotter 接口并冻结独立 gate，再做源年筛选 |
...
4. 若安全约束仍无法满足，核验 L10 的原件和接口，再筛选 L11 双玩家约束；双玩家层不能绕过独立 FPR gate。
5. L15--L24 只在前述最小路线没有足够信号且资源预算允许时进入；课程、强化学习和攻击者博弈保持条件状态。
6. 任何候选通过来源／开发层后，才允许冻结一次 `2018--2025` 封印评价；目标期结果不得回灌台账排序。
