...
4. L11 双玩家 v2 已按源年筛查否决（单臂未介入、联合臂不通过 FPR 与旧恶意保持门，见上表）；安全约束候选按演化链进入 L28 尾锚（先过 E1 oracle 判读，再跑 ER2018 四臂）；双玩家／尾锚层都不能绕过独立 FPR gate。
5. L15--L24 只在前述最小路线没有足够信号且资源预算允许时进入；强化学习和攻击者博弈保持条件状态，课程学习（台账 L25）仅在尾锚失败且证据指向序贯调度时按 CUR 部件槽重开。
