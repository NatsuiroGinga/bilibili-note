...
## 17. 官方 24M 三臂 rerun 的四条设计答复（2026-09-10）

对象：24.2M 双分支 Transformer、T17 train 抽样 6 万、3 epochs、batch 128、Adam 分层 lr（骨干 1e-6/头 1e-4）；臂 A 干净／B 干净+CharBot 2 位替换（1:1）／C 组相对加权（K=4，骗过 2.0／未骗过 0.5）。

**Q1 训练进度折算（本代理计算，非论文原文数字）**
- Drichel（§4.1 p.5、§4.2 p.5、Fig 1–2 p.7、§4.4 p.7）：数据集约 25 万平衡样本；每折训练集 75% ≈ **18.75 万**；minibatch **512 = 256 干净 + 256 对抗**；**最多 50 epochs**。
- 每 epoch 样本呈现数 ≈ 18.75 万干净 + 18.75 万对抗 ≈ **37.5 万**；50 epochs ≈ **1,875 万**。
- 本课题：6 万 × (1 干净 + 1 对抗) = 每 epoch 12 万；3 epochs = **36 万**。
- **比值 ≈ 1.9%（按样本呈现数）／6%（按 epoch 数）——差一个数量级。**
- **但 Drichel 的 50 是"因早停失效而设的上限"，不是实测收敛点**：原文只写 "the validation loss tends to increase for the first epochs before decreasing again"（§4.4 p.7），**从未给出验证损失开始下降的 epoch**。故"3 epochs 是否停在上升段"**Drichel 无法裁决**；"最小可接受 epochs"**无文献依据**。
- 可操作的迁移纪律：**不要用验证损失决定何时停**，改固定 epoch 预算；**但必须逐 epoch 记录干净 FPR 而不只记最后一轮**。若第 3 轮干净 FPR 仍未回落到 A 臂水平且呈上升趋势，即为"训练段未走完"的直接证据，应延长而非判定机制失败。

**Q2 配比**：1:1 读法**确认**（Fig 1 与 Fig 2，p.7，512 = 256 "Benign Samples" + 256 对抗）。对抗半批内部 7 分的**原文理由**是 "**To not under-represent** MaskDGA-WB and HotFlip"（§4.4.2，p.7）——**目的是防止某来源被饿死，不是难度加权**。单一算子下按扰动位置分桶是该原则的**类比**（Drichel 分的是攻击种类，不是同一攻击内的位置），须登记为**任务化设定，非复现**。

**Q3 扰动预算**：Drichel §4.4.1（p.7）确引 Kurakin [33] "training against one pre-set perturbation budget does not improve robustness against other perturbation budgets to the same degree"，并给出自己的理由（无界预算会让攻击随机初始化到允许空间任一点、失去与训练数据的联系）。§1（p.2）另有明确警告：**只针对小扰动硬化会给出 false sense of security**——"classifiers hardened with these AEs could still be fooled by an adversary performing the same attack with an increased bound"。
- **建议：P0 的 2 位不动**（病灶复现探针须与 CharBot 原文可比）；**训练臂可扩，但必须把"训练扰动位数"与"评价扰动位数"分开登记**，评价至少在 **k=2（对齐 P0）** 与 **k∈{1..4}（预算泛化）** 两组上报。
- **归因风险提示**：若把臂 B 从"固定 2 位"改为"k~U{1..4}"，则 **B vs A 同时改了两件事**（引入对抗样本 + 改变预算），P2 的读数将无法区分"增广生效"与"预算变宽生效"。宜另设 B2（固定 k=2）作对照，或先把预算固定、把随机化留到 P3 之后。

**Q4 组相对加权**
- **Drichel 只用均匀混合**（§4.4.2，p.7，每种攻击等量），**无任何难度加权**。
- 已知的"按难度筛选/加权"先例（均为**采样或过滤**，非连续权重）：DAPO 动态采样（剔零优势组）、SRPO History Resampling（**剔全对、留全错**，方向与直觉相反）、GFPO（按目标属性拒绝采样，优势只在保留子集内算）。
- **MAB-Malware 不可用作依据**：其"按逃逸概率选择"是**攻击者侧策略**（决定对哪些样本施加哪个动作），**不是防御者侧的样本权重**；援引它论证防御侧加权属**类别错误**。
- **结论（用户 2026-09-10 裁决，覆盖原措辞）：防御侧"按变体是否骗过当前模型"逐样本加权，在本代理已读文献中无先例——登记为「无先例的任务化机制（新颖性空间）」**：GRPO 家族的组相对思想（**组内相对优势替代绝对损失阈值**）从 LLM token 级迁移到**扰动组级**，这正是本方案的**机制提取**；"无先例"是**新颖性空间本身**（**Drichel 的均匀混合 ＋ 本方案的加权 = 差量**），**不是依据缺陷**。其 2.0/0.5（4:1）权重比同样无先例。
- **该机制不需要文献背书作为"标准做法"**：它的验证方式就是 **P3 判据实验**（加权后**对抗检出改善 且 干净 FPR 不增**）。因此登记措辞**必须同时写两面**——「**无先例（新颖性空间）**」＋「**实验验证路径（P3 判据）**」，两者缺一即失真。
- **与"问题不存在的机制"不同类（EATA 结论不变）**：EATA 式熵阈值属"**所解决的问题不存在**"（有监督对抗训练下，"该变体是否骗过当前模型"是直接可观测量），故**不加**的结论不变；本条的组相对加权属"**问题真实存在但无先例**"的创新机制，登记为待验证差量。两类不可混用同一套登记措辞。
- **"过度拟合难样本"陷阱**：本代理已读文献中**未找到**对该陷阱的直接证据，**不裁定**。可登记的**推论**是：以"骗过当前模型"为权重会使训练分布**跟踪模型的瞬时弱点**，而 Drichel §1（p.2）警告的正是"只覆盖训练期攻击/预算的硬化会给出虚假安全感"。
- **EATA 式熵阈值不建议加**：EATA 的熵阈值用于**标签缺失**时筛"可靠样本"；本臂是**有监督对抗训练，"该变体是否骗过当前模型"是直接可观测量**，不存在它要解决的问题。若仍要加，须先说明它替代了什么缺失信号。
