...
- [ ] 阶段二：冻结通用文档、collection、权威、状态、证据和敏感度元数据合同。
- [ ] 阶段三：实现安全扫描、类型化分块、单库模式、增量复用和 collection 陈旧收据。
- [ ] 阶段四：实现 `paper/project/experiment/thesis/all` 路由、collection 过滤、分区输出与论文证据隔离。
- [ ] 阶段五：更新 README、根触发规则和混合检索技能。
- [ ] 阶段六：执行一次最小编译、导入、CLI、真实小规模构建和作用域查询，记录语料、大小与延迟。
- [ ] 阶段七：检查共享暂存区，仅在为空时提交本任务独占文件。
