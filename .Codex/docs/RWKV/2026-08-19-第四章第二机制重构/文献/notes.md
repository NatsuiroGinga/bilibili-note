...
- **最强领域近邻**：Daum 等的 ECHO。其物理第 6 页用多出口分类器和置信度门提前退出，物理第 10—12 页观察到部分流“早期判对、后来判错”。它能证明加密流量存在早期证据消失现象，但不能证明本课题冻结 XGBoost 路径存在同一现象。
...
| 方向 | 最近全文 | 裁定 | 原因 |
| --- | --- | --- | --- |
| 反射累积证据首达 | Huang—Veeravalli；Wu 等 | 有条件 D2 | 能接收冻结分数经预注册变换后的增量，且与运行均值不同；但变点/独立假设和增量构造必须实测，风险阈值需重新 Tong 校准 |
...
5. 只有当 M1 后仍在长桶出现可重复的 FPR/DR 或时延异常，且路径显示“旧前缀稀释后段持续证据”或可审计的负/正漂移时，才进入反射累积证据试验。
