...
- 推荐“Raw83 唯一拟合源＋共享臂级物化”：LSPR23 物化候选 A、B 两份共享 `float32` 视图；输入、配方、容量、检查点、机制和评价代码全部完成源侧资格封印后，LSPR24 才物化胜出臂一份视图。
...
- Raw83 全量缺失、非有限、端口/二元/协议结构非法、有限负值和时间解析计数。
- 两个 LSPR23 共享视图及胜出 LSPR24 视图的真实内容 SHA、吞吐和峰值资源。
- 官方单次行主序噪声通过磁盘映射 `out` 生成时，与同版本内存一次性生成的逐位一致性。

## 设计自审结果

- 首轮草案曾允许源输入臂封印前读取 LSPR24 无标签特征；自审判定这仍会暴露目标分布，已修正为输入、配方、容量、检查点、机制和评价代码全部完成源侧资格封印后，才允许打开 LSPR24 的模式、特征或标签。
- 候选 B 已明确标成“官方参数的任务适配”，因为七特殊字段旁路不是官方源码行为。
- 83 字段表与 `dijk_fields.DIJK_FEATURES` 自动对比零差异；资源数字已独立复算。

## 2026-08-22 数据语义复核修订

- 主进程复核指出，首版 3.3 把计数、标志、时长、IAT 和速率字段的负值一概转缺失，缺少生成器原始规范支撑。既有共同字段审计明确记录 LSPR24 `Bwd Bulk Rate Avg` 含负值，因此原规则会在共享层擅自改变可解析有限数据。
- 已修正为默认保留所有可解析有限数值，包括负值。只对端口整数及 `[0,65535]`、索引 `80..82` 的 `{0,1}` 二元定义、索引 `2/79` 的可解析整数定义做结构强校验。
- 其他字段只记录最小值、最大值和负/零/正计数，不据字段名清洗。任何未来范围清洗都必须有已核原始生成器规范并提升模式版本；`finite_negative_policy=preserve_and_count-v1` 必须进入模式 SHA。

## 2026-08-22 `c581a12` 唯一修订复审证据

- 审查裁决是 `REQUEST_CHANGES`，不是方案或模型族否决。主要问题是冻结身份、原子发布、目标年物理分离、能力缓存和有限性门不完整。
- 冻结 SHA 必须来自主进程后续提供的真实旧制品收据。未提供前实现只能强制字段和校验逻辑，不能写假 SHA。
- 目标配置必须固定 Parquet 字节数、SHA-256、物理模式 SHA、源资格封印内容 SHA 和唯一胜出臂。源配置、源工具和源启动器不得含任何 LSPR24 路径或字面年度名。
- P2 的“累计有效位恰等于流数”必须在每批先对有效行号 `unique` 后再写全局覆盖位图，不能用 NumPy 重复花式索引的结果猜测重复。
- 标签缓存只允许进程内约 `16 MiB int8`；不是磁盘制品，也不得跨运行复用。
- 临时 B 臂噪声与 filled 映射只有在 P5 视图、清单和发布指针全部封印后才可精确删除；任何失败保留用于诊断。
