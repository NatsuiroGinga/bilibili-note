# XGBoost 加两机制实施计划

## 目标

在 B76 上完成 XGBoost 的 CPA、ELP 及组合四格真实实验，以源年实体折外协议选择 CPA 挂载和幂次，并生成可复核的聚合制品。

## 文件边界

- 新增 `thesis/experiments/llm_probe/tools/ch3_xgb_cpa_elp_entity_oof.py`
- 新增 `thesis/experiments/llm_probe/configs/ch3-xgb-cpa-elp-gpu-oof-seed42-v1.json`
- 新增 `thesis/experiments/llm_probe/scripts/remote_launchers/run_ch3_xgb_cpa_elp_gpu_oof_seed42_v1.sh`
- 更新本目录 `notes.md` 和最终运行报告
- 不修改既有 XGBoost 锚点脚本、第三章正文、共同缓存或正在运行的卷积实验

## 阶段

- [x] P0：读取规则、执行文档、文献审计、接口勘察和真实缓存
- [x] P0：核验 XGBoost 3.2.0 官方 GPU 接口与 `E23` 实体分组
- [x] P0：修订执行合同，删除实体泄漏、CPU 逐位门和逐样本持久化问题
- [x] P1：实现单入口工具、冻结配置和持久启动器
- [x] P1：执行语法、配置、命令入口和 Shell 检查
- [x] P1：同步 B76；用户依据实时资源明确允许与卷积并行，已启动持久会话
- [ ] P1：修复有效超参浮点精确相等门禁，用新身份重启并完成 11 次拟合
- [ ] P0：补齐折模型与最终模型的可验证恢复入口；完成前禁止因失败新开全量重跑
- [ ] P1：回收聚合制品，执行结果分析并更新恢复文档
- [ ] P2：按原子范围提交代码、配置、启动器、合同和报告

## 当前状态

运行身份为 `ch3-xgb-cpa-elp-gpu-oof-seed42-v1`，持久会话为
`ch3-xgb-cpa-elp-gpu-s42-v1`。首次运行已停止，状态为
`failed/experiment/python_failed/exit_code=1`。源年实体、三折隔离、时间非降、`mean166` 与
`semantic168` 均已完成；首个 `raw83/fold0` 已训练 `800` 轮，用时约 `0.30` 分钟。随后有效
超参核验把回读值 `0.800000012` 与配置值 `0.8` 作精确相等比较，`subsample` 与
`colsample_bytree` 两项被误判为不一致并终止。有效拟合仍为 `0/11`，适配器与 `p` 均未选择，
LSPR24 未读取，无科学指标。旧运行只作工程失败证据，实验待证。

## 阻塞

- 当前阻塞为有效超参收据的浮点比较实现。只需将精确相等改为预先固定的绝对／相对容差比较，
  不改变 XGBoost 超参、数据、折、适配器、ELP 网格或评价协议。
- 资源不是失败原因。失败前快照为 GPU 使用／总量 `9842/32607 MiB`、空闲 `22268 MiB`、利用率约
  `34%`；控制组内存使用 `52,524,875,776` 字节，上限 `96,636,764,160` 字节。

## 错误记录

- 首次远端形状核验尝试以 `mmap_mode` 打开对象数组，NumPy 拒绝对象类型内存映射；已缩小为数值缓存核验，不影响数据或实验。
- 首次正式运行在 `raw83/fold0` 完成训练后误停：XGBoost 将配置的 `0.8` 回读为
  `0.800000012`，精确相等门禁错误地判为超参漂移。LSPR24 未读取，故不污染源年选择协议；
  修复后须使用新运行身份，不覆盖失败根。
- 生产实现保存九个折模型与两个最终模型，但没有加载、核验和跳过已完成单元的恢复入口；启动器反而在
  同名运行目录存在时直接退出。该实现违反局部规则的长任务检查点要求及执行文档第 174 行，先前审查把它
  降为非阻断 `P1` 属错误分类。后续任何失败重启前必须先关闭该项。
