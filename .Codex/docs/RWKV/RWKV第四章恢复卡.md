...
3. 机制一首选 Q0 使用实体路径最大统计量做一次实体级纽曼—皮尔逊校准：校准折把每个实体全部已记录前缀压成一个最大分数，评价时仍逐曝光首次越界告警。该规则不需要曝光截断或人为预算衰减率，但只保证到 LSPR23 已记录路径支持，不外推无限寿命实体。
4. `α_j=α/[j(j+1)]` 只作逐曝光预算压力对照，不作主机制；当前没有已核全文直接覆盖同一实体相关前缀的该构造。机制一 Q0 比较首曝、旧固定阈值全曝光、路径最大校准和上述压力对照，仍只使用 LSPR23、种子 42、零训练。
5. 路径最大校准只有在实体假阳率不超过 `4%` 且检出率严格高于首曝基线时才可晋级；否则停止该机制，不调整预算形状或路径支持。
