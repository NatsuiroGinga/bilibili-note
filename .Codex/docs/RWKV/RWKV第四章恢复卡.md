...
- **总证据等级**：固定二进多尺度 DTEP、无标签 BBSE-PBC 与机制二有限样本确认均已完成有效单种子 Q0 且不晋级；机制一的 LSPR23 重复曝光联合病灶已通过资格诊断，Tong 有限样本同模型实体路径最大控制已通过正式修正源年 Q0。机制一当前证据等级为源年单种子 Q0 实验支持，不是跨年度或正式论文证据；不得写成第四章双机制候选已支持。
...
- 机制一已取得“病灶存在”资格，并已通过正式修正源年 Q0 的 Tong 有限样本实体路径最大控制；当前只得到源年单种子 Q0 实验支持。机制二当前实现已否决，因此尚不存在可命名的第四章双机制算法。
