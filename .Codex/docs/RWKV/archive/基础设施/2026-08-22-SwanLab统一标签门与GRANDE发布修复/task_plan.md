...
4. 扫描器对真实 JSON/YAML 配置中的 `swanlab` 与 `tracking` 节点调用同一合同函数，并列出剩余直接初始化入口；扫描报告不自动改写旧配置或源码。
...
UV_CACHE_DIR=/tmp/uv-cache PYTHONPYCACHEPREFIX=/tmp/codex-pycache uv run --no-sync --with pyyaml==6.0.3 python tools/validate_swanlab_contracts.py --config-root configs --alias-config configs/swanlab-tag-aliases-v1.json --output /tmp/swanlab-contract-migration-gate-v2.json
...
- [x] 完成态 manifest、登记输出与生产身份全量复核。
- [x] 父 manifest 23 项反向核验及精确三项历史漂移合同。
- [x] JSON/YAML 与 `swanlab`/`tracking` 完整迁移门。
- [x] 目的地正则、长度、模式映射、版本和别名 SHA 收据。
- [x] 首次零步初始化 401 的全新进程单次重试。
- [x] `publish_completed`、单次 `finish()` 和失败 manifest。
- [x] SwanLab 目的地、别名路径和修复配置哈希冻结。
- [x] 文档声明收窄到已接入入口和 GRANDE 修复。
