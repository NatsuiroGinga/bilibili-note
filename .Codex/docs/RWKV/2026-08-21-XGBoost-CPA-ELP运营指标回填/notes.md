...
- 可复用的实现事实包括 168 维构造公式、LSPR24 冻结规模、时间顺序核验、CUDA 模型配置和实体键构造。
...
- MLP/RWKV 实现把实体第一条合法流记为曝光 1，在线 C11 分数为前缀幂平均；`p=1` 时即前缀算术平均。达到同档完整并列阈值时首次告警，按时曲线分母固定为全部正实体，未告警实体作为删失保留。
- RWKV 实现的分位数为 `minimum/p25/median/p75/p90/p95/p99/maximum`，按时曲线采用右连续精确断点，并包含横轴 0 与最大正实体曝光数。

## 本任务采用的差异

- 只输出目标年 C11，不输出源年或四格。
- 只加载七个 LSPR24 数组；`X24` 作为 83 个基础字段进入 `semantic168` 构造，但不形成 `raw83` 评价分支。
- 只加载 `model_semantic168.json`，目标 GPU 分数调用计数必须严格为 1。
- 逐流分数、最终实体分数和逐实体首次告警位置只在内存存在，完成聚合后立即释放，不进入 NPZ 或 JSON。
- `t24` 只用于顺序核验；它不是完整流 `available_ns`，所以时间时延不可用。

## 运行身份建议

- 运行标识：`ch3-xgb-cpa-elp-c11-operational-backfill-v1`。
- 展示名称：`XGBoost＋CPA-ELP C11目标年完整运营指标零训练回填`。
- 输出根：`runs/diagnostics/ch3-xgb-cpa-elp-c11-operational-backfill-v1/`。
...
- 本轮不访问服务器，无法验证父制品和缓存是否仍在远端，也不产生实际运营指标。
