...
- 旧四格的 `C01p=C11p` 与 CPA 无关闭格问题已由提交 `ab9ed32` 标为 `INVALID_REVOKED_DO_NOT_EXECUTE`；该问题已关闭，不再要求在旧公式上修补。
- 用户批准的新冻结矩阵见提交 `a3a4cd4`：因子一为既有 CPA 检查点 `B00/B10`，因子二为实体算子 `max` 与固定 `0.5×mean+0.5×max`。四格共享同一父检查点逐流输出，因子可识别。
- 这一 S0 只读 LSPR23，不读取 LSPR24，不训练、不搜索 `0.5/0.5`。固定等权是任务化设定，没有直接全文依据；其价值来自最低成本可证伪，不来自文献证明。
- 新 S0 只能回答“固定 mean-max 混合是否与既有 CPA 在源年实体运营指标上形成信号”。它**不能**回答有限 `Lp + max` 是否有效，也**不能**回答新分数层 CPA 是否有效，因为两者均未实现。
- 报告实体 AP、最大实体 AP、完整可达整并列组曲线、六档实际 FPR、首次告警、未告警率、曝光序阶梯、逐流同一性与资源。若固定混合无信号或产生低预算新退化，停止该代理方向，不进入训练层。
- 只有该 S0 出现源年真实非支配信号，才允许另立冻结骨干 S1 讨论学习型联合头；届时仍须把 `Lp`、`max`、联合头和同容量对照分开，不能把本轮 mean-max 结果冒充 `Lp + max` 证据。
