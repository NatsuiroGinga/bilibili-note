...
| 补检索方向 | 结果 | 证据等级 |
| --- | --- | --- |
| multi-quantile / simultaneous quantile estimation / non-crossing quantiles | **成熟领域**，见 C1–C2。核心问题就是「多个水平的次序与间距」 | `在线摘要`＋正文片段，**无本地原件** |
| multi-level / multi-threshold risk measures；joint optimization over multiple CVaR levels | **成熟领域**：Kusuoka 表示、mixed-quantile quadrangle | `在线摘要` |
| **spectral risk measures**（对 α 加权积分＝多水平连续极限） | **成熟领域且有专门算法**：SOREL (Ge 2024)、Prospect (Mehta 2023)、SRCPO (Kim 2024)、Moghimi 2025、Chu 2021、Fröhlich 2022 | `在线摘要` |
| multi-quantile 的**分阶段/课程式**训练（先中位数后极端分位数） | **未命中任何一篇** | — |
| 多预算 pAUC / 多 FPR 工作点的联合训练与**调度** | 联合训练有（Yao 2022、Zhu 2022、Jiang 2025，均在库）；**调度未命中** | 本地全文（联合训练部分） |
| curriculum over loss levels / annealing quantile levels in deep learning | 命中的「课程」全部作用于**数据难度**（Açıkgözoğlu 2026 攻击强度 `15%→1%`；CLRe 分子复杂度），**不是损失的风险水平** | `在线摘要` |

### 对「谱风险测度里有没有逐步引入水平」的专项回答

专项检索（alphaXiv `discover_papers` 两轮、Consensus 两轮、Scholar Gateway 两轮）
**未命中任何「按水平逐步引入」或「从粗到细的水平网格」的谱风险测度训练方案**。
最接近的一篇是 `alphaXiv 2512.22986` *Risk-Averse Learning with Varying Risk Levels*（2025-12-28，
`在线摘要`）：**单个**风险水平随环境演化而变的在线优化，**不是多水平分阶段引入**。

### 三选一裁断：**有邻近但不对位**

- **「多档并列」有先例且成熟**（谱风险测度／Kusuoka／混合 CVaR／复合分位数回归／multi-α learning）。
  本课题的六档等权平均**就是**一个离散谱风险测度。**这一半不是空白。**
- **「多档分阶段引入」本次未命中先例**。但这是**检索未命中**，**不是存在性证明**；
  且本次全部相关证据只到 `在线摘要`／正文片段，**按本仓库证据门槛不得据以下「无先例」的定案裁断**。
- **最有判别力的一点不在有无先例，而在解法选择**：真正研究过「多个水平互相干扰」的文献
  （不交叉分位数一支）**选择了次序约束与单调参数化，没有一篇选课程**。

### 对「理由三」的结论

**理由三不成立**——但不是因为找到了反例，而是因为**它的证据基础本来就不成立**：

1. 2026-09-03 那次检索**用错了词表**。本课题在「多预算 pAUC」这个自造词下检索，
   完全没有触及 spectral risk measure／Kusuoka／mixed-CVaR／composite quantile regression
   这几个**该结构的标准名字**。**「无先例」是词表覆盖不到，不是文献没有。**
2. 2026-09-04 的补齐只加了两篇**单水平**论文，**没有修正词表**，
   故「多档 staging 仍无先例」这一结论**继承了同一个词表缺陷**，未获独立支持。
3. 本次在正确词表下检索，发现**并列结构是成熟的、分阶段引入未命中**——
   即「新颖性」只可能落在**调度**这一层，而调度层恰恰是判一证明**两篇先例都撑不住**的那一层。

**故：理由三（文献无先例）应判为「未成立且不可作为存废依据」，
而非主代理 §7.1 的「仍然成立」。** 它既不能用来降级本候选（因为无先例不等于错），
也**不能用来抬高本候选的新颖性**（因为并列结构一点也不新）。

## 阻断项（须主代理处置）

1. **证据等级不足以定案**：判三的全部关键文献（SOREL、Prospect、SRCPO、Barrera 2025、
   Moon 2021、Cannon 2018）**均无 `raw/` 原件与 `wiki/` 全文笔记**，只到 `在线摘要`。
   按本仓库门槛，**不得据此写入正文或路线总控**。
   **本代理不下载文件**（下载属需用户明确授权的动作），入库须由主代理决定并执行。
   建议优先入库顺序：Barrera et al. 2025（`doi:10.1111/mafi.70000`，直接对位「多水平 vs 单水平」）
   → Mehta et al. Prospect 2023（谱风险 DRO，含 average top-k 与 CVaR）
   → Kim et al. SRCPO 2024（多对偶变量结构与六个 `ξ` 同构）。
2. **Elicit 来源整轮不可用**（`api_access_denied`），本次调研缺该来源覆盖。
3. **本地语料在谱风险／多分位数一支为空白**（891 篇 papers，两次 hybrid 查询零相关命中），
   Zotero 同样为空白（12/12 相似度为负）。**今后此类判断不能只靠本地与 Zotero。**
