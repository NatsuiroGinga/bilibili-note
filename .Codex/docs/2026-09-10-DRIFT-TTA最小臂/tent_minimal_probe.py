...
def main(limit: int | None = None, lr: float = LR) -> None:
    ...
    print(f"[TENT] T18 test 流：{len(domains)} 域（无标签适应，标签仅事后评价）", file=sys.stderr, flush=True)
    ...
    # 适应后 state_dict 原子落盘
    sd_path = Path(f"/tmp/drift-tent-t18-lr{lr}{'-dry' if limit else ''}.pt")
    ...
    out_name = "tent-result-DRYRUN.json" if limit else "tent-result.json"
