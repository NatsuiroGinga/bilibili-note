...
5. 训练前数据门拒绝非正批量、空训练集、训练域和标签数量不一致、零批次，以及未同时且仅含标签 0/1 的训练数据。
6. 每臂以非阻塞 `fcntl.flock` 保护断点加载、训练、该臂目标评价及 `result_{arm}.json` 原子写入；已有同臂写者会立即报出锁文件路径并停止。无论任一臂或汇总写入是否异常，`finally` 都会调用 `run.finish()`。
...
- 通过静态检索：CPU 映射加载、设备与 CUDA RNG 数量合同、RNG 类型与数量检查、严格臂解析、训练前数据门、非阻塞锁、独立汇总名和 `run.finish()` 的 `finally` 均存在；旧的 `ckpt_path.unlink()` 与共享 `result_all.json` 写入已不存在。
