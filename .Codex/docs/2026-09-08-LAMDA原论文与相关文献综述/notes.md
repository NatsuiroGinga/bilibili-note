...
- 2026-09-09 新增并完成本地原件核验：PCT（arXiv `2011.09161`）、Cotter Proxy-Lagrangian（arXiv `1809.04198`）、Cotter 独立验证集约束（arXiv `1807.00028`）和 Kumar ICO（arXiv `2107.10960`）。四个 PDF 已下载、SHA-256 已登记，MinerU 完整或分段解析文件位于 `.Codex/docs/2026-09-09-LAMDA-PCT-FPR门控/source-extract/`，对应 `wiki/papers/attack-detection/` 结构化笔记已建立。它们只提供方法来源和对照设计，不提供 LAMDA 本地效果。
