# GitHub 与 Hugging Face 候选检索通道实施计划

## 目标

在现有本地论文证据与在线论文候选之外，新增 GitHub 代码候选、Hugging Face 论文候选和 Hub 候选。GitHub 输出为 `code_candidates`，HF 模型、数据集与 Space 输出为 `hub_candidates`；两者均不进入本地论文 RRF。HF Papers 作为论文 provider 进入 `paper_candidates`。
...
7. HF 公开读取允许无令牌；可选 `HF_TOKEN` 只进认证头，不输出。Hub 候选返回仓库 ID、类型、任务、下载、点赞、更新时间、门控、许可证、arXiv 关联、链接依据与核验状态。
8. 普通 GitHub/HF 搜索固定为 `unverified_code_candidate` 或 `unverified_hub_candidate`；只有来源元数据显式带论文 arXiv 关联时可标 `paper_linked_candidate`，本工具不自动升级或下载。

## 验证

1. 核验 GitHub 官方仓库搜索、认证、速率限制和许可证字段文档。
2. Python 编译、模块导入、CLI 帮助与 README/技能路径检查各一次。
3. 使用 `GRANDE` 和 `RWKV` 对 GitHub 与 HF 各执行真实联网查询，记录认证、限流、结果结构和去重。
...
- [x] 官方接口核验
- [x] 实现独立 GitHub/HF 通道
- [ ] 真实联网验证：安全审批拒绝向外部服务发送 GRANDE/RWKV 研究主题，未绕过
- [ ] 报告与提交
