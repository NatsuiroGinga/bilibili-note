...
printf 'state=running\nstarted_at=%s\nmodel=hgb\n' "$(date -Is)" >"$launcher_dir/hgb-batch.status.tmp"
mv "$launcher_dir/hgb-batch.status.tmp" "$launcher_dir/hgb-batch.status"

for seed in 42 43 44; do
  output_dir="runs/baselines/theory-selection/genis-v0/family-hgb-seed${seed}"
  status_file="$launcher_dir/hgb-seed${seed}.status"
  command_file="$launcher_dir/hgb-seed${seed}.command.txt"
  log_file="$launcher_dir/hgb-seed${seed}.launcher.log"
  ...
  printf '%s\n' \
    ...
    "  --seed $seed" \
    "  --run-name theory-selection-genis-family-hgb-seed${seed}" >"$command_file"
  ...
  uv run --no-sync flow-probe-tabular-baselines \
    ...
    --output-dir "$output_dir" \
    --seed "$seed" \
    --run-name "theory-selection-genis-family-hgb-seed${seed}" >"$log_file" 2>&1
  exit_code=$?
