...
class RunNodeHookTest(unittest.TestCase):
    ...
    def test_allows_security_guard(self) -> None:
        node = shutil.which("node")
        self.assertIsNotNone(node)
        assert node is not None
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            launcher = root / LAUNCHER.name
            shutil.copy2(LAUNCHER, launcher)
            (root / "security-guard.js").write_text(
                "process.stdout.write('安全守卫已运行');",
                encoding="utf-8",
            )
            env = os.environ.copy()
            env["PATH"] = f"{Path(node).parent}:/usr/bin:/bin"
        ...
        self.assertEqual(result.stdout, "安全守卫已运行")
        self.assertEqual(result.stderr, "")

    def test_rejects_unknown_hook_name(self) -> None:
        result = subprocess.run(
            ["/bin/sh", str(LAUNCHER), "unknown-hook.js"],
            text=True,
            capture_output=True,
            check=False,
        )
