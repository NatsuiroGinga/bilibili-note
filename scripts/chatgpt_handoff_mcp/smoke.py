...
def initialize_and_create(project: Path) -> None:
    ...
    run(
        ...
        "experiment-analysis",
        "--sanitized-summary",
        "源期聚合指标：模型 A 的召回率 0.81，模型 B 的召回率 0.79。",
        "--model",
        "research-model",
        "--effort",
        "high",
    ...
    rejected_cases = (
        ("absolute-path", "个人绝对路径", ["--task", "请读取 /Users/example/private.md"]),
        ("raw-data", "原始数据", ["--task", "请分析原始数据"]),
        ("dataset", "数据集", ["--task", "请分析数据集"]),
        ("checkpoint", "检查点", ["--task", "请分析检查点"]),
        ("weights", "权重", ["--task", "请分析模型权重"]),
        ("target-period", "目标期信息", ["--task", "请分析目标期表现"]),
        ("log", "日志", ["--task", "只读分析", "--sanitized-summary", "日志显示模型 A 更好"]),
        ("credential", "凭据", ["--task", "请读取令牌"]),
        ("server", "服务器连接", ["--task", "请通过 ssh 连接"]),
    )
    for name, label, arguments in rejected_cases:
        result = run(*base, "--name", f"rejected-{name}", *arguments, check=False)
...
async def verify_mcp(project: Path) -> None:
    ...
    async with stdio_client(parameters) as streams:
        async with ClientSession(*streams) as session:
            ...
            if manifest["task_type"] != "experiment-analysis":
                raise AssertionError("任务类别字段未往返")
            if manifest["sanitized_summary_included"] != "true":
                raise AssertionError("脱敏摘要标记未往返")
...
async def main() -> None:
    ...
    try:
        ...
        print("脱敏源期正例、敏感负例、MCP 往返与越界拒绝通过")
    finally:
